...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        # bg - фон
        # bd - границы
        toolbar = tk.Frame(bg='#778899', bd=0)
        # упаковка
        ...
        # image - иконка кнопки
        btn_open_dialog = tk.Button(toolbar, bg='#778899', bd=2,
        ...
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'tel', 'email','Money'),
        ...
        self.tree.heading("email", text='E-mail или G-mail')
    ...
    def update_record(self, name, tel, email, Money):
        self.db.c.execute('''UPDATE db SET name=?, tel=?, email=?, Money=? WHERE ID=?''',
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        label_sum = tk.Label(self, text='E-mail или G-mail')
        ...
        # добавляем строку ввода для email-gmail
        ...
        self.entry_Money.place(x=200, y=140)
...
class DB:
    def __init__(self):
        ...
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text, Money text)''')
        # сохранение изменений БД
        self.conn.commit()

    # метод добавления в БД
    def insert_data(self, name, tel, email, Money):
        self.c.execute('''INSERT INTO db (name, tel, email, Money) VALUES (?, ?, ?, ?)''',
                       (name, tel, email, Money))
        self.conn.commit()
